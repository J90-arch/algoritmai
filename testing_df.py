...
def AI_prediction():
    ...
    model.load_weights("results\SPY-sh-1-sc-1-sbd-0-huber_loss-adam-LSTM-seq-50-step-15-layers-2-units-256.h5")
    
    current_price = si.get_live_price(stock_short_name)

    data = load_data(stock_short_name, N_STEPS, scale=SCALE, split_by_date=SPLIT_BY_DATE, 
    ...
    print(f'future price {future_price}, current price {current_price}')
    if delta > 0.001:
        return True
    elif delta < -0.001:
        return False
